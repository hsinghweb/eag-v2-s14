...
def render_graph(steps_dict, depth=1):
    """
    Refactored to accept 'steps_dict' (Dict[str, StepNode])
    ...
    # Iterate over the manual dictionary
    for node_id, node in steps_dict.items():
...
def save_final_plan(session_id: str, final_data: dict, base_dir: str = "memory/session_logs"):
    # Fix import error if not in scope
    from datetime import datetime
    folder = get_log_folder(session_id, base_dir)
