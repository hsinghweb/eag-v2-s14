...
from browserMCP.browser import BrowserSession
	...
	def __init__(
		...
		async def click_element_by_index(params: ClickElementAction, browser_session: BrowserSession):
			...
			try:
				...
				# 🚨 NEW: Enhanced post-click timing and stabilization
				if download_path:
				...
				# Wait for page changes to complete
				await asyncio.sleep(0.5)  # Initial wait for DOM updates
				...
				# Force element map refresh to get new IDs
				await browser_session._refresh_element_map()
		...
		async def input_text_by_index(params: InputTextAction, browser_session: BrowserSession, has_sensitive_data: bool = False):
			if params.index not in await browser_session.get_selector_map():
			...
			# 🆕 ADD: Wait for input validation
			await asyncio.sleep(0.5)  # Allow validation to complete
